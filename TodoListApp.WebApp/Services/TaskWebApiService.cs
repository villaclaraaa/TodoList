...
public class TaskWebApiService : ITaskWebApiService
{
    ...
    public async Task<IEnumerable<TodoTask>> AssignedTasks(int userId, int page, int pageSize,
        TaskSortOption sortBy, bool descending)
    {
        try
        {
    ...
    public async Task<int> GetAssignedTasksCountAsync(int userId)
    {
        try
        {
            var response = await this._httpClient.GetAsync($"api/Task/assigned/{userId}/count");
            response.EnsureSuccessStatusCode();
