...
namespace TodoListApp.WebApi.Controllers
{
    ...
    public class TaskController : Controller
    {
            ...
            [FromQuery] bool descending = false)
        {
            try
            {
                var tasksList = await this._taskService.GetTasksAssignedToUser(userId, page, pageSize, sortOption, descending);
                if (tasksList == null)
                {
        ...
        public async Task<ActionResult<int>> GetAssignedTasksCount(int userId)
        {
            try
            {
                var count = await this._taskService.GetAssignedTasksCount(userId);
